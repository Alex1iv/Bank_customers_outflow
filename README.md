...
## Summary
It was predicted the client outflow from a certain bank using the ensamble ML model. Several model were compared and the Random Forest model showed satisfactory result (F1-score=0.82) even after reducing the number of features from 11 to 4. User may predict customer status by a given data on a web page on the local machine, which can be generated by a micro service application.
